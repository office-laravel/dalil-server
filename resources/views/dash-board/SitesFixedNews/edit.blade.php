...
@include('dash-board.layout.sidebar')
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg overflow-x-hidden">
        ...
        <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl" id="navbarBlur"
            ...
            <div class="container-fluid py-1 px-3">
                <nav aria-label="breadcrumb">
                    ...
                    <h6 class="font-weight-bolder mb-0">مواقع مثبتة للاخبار</h6>
                </nav>
        ...
        <div class="row backgroundW p-4 m-3">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('fixedsitesnews.main') }}">الرئيسية</a></li>
                    <li class="breadcrumb-item active" aria-current="page">انشاء</li>
                </ol>
            </nav>
            <form action="{{ route('fixedsitesnews.update' , $fixedsitestnews->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                
                <div class="form-group">
                    <label for="exampleFormControlInput1">اسم الموقع</label>
                    <input type="text" class="form-controll" name="name" value="{{$fixedsitestnews->name}}">
                </div>
                
                <div class="form-group">
                    <label for="exampleFormControlInput1">رايط الموقع</label>
                    <input type="text" class="form-controll" name="href"  value="{{$fixedsitestnews->link}}">
                </div>

                <div class="form-group">
                    <label for="exampleFormControlInput1">صورة</label>
                    <input type="file" name="photo" class="form-controll">
                    <img src="{{url('../public/uploading/' . $fixedsitestnews->img)}}" alt="{{$fixedsitestnews->link}}">
