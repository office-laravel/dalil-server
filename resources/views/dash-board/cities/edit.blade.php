        ...
        <form action="{{ route('city.update', $cities->id) }}" method="POST" >
            ...
            <select style="width:100%;overflow-y: scroll; padding:5px;" class="form-cdontroll" id="country_id"
                    ...
                    @foreach ($country as $val)
                        <option value="{{ $val->id }}" {{$val->id == $cities->country_id ? 'selected' : ''}}>
                            {{ $val->country_name }}
                        </option>
                    @endforeach
                </select>
            <div class="col-md-12 mb-3">
                ...
                <input type="text" name="name" value="{{$cities->name}}" class="form-controll">
            </div>
            <div class="col-md-12 mb-3">
                <label for="Description" class="form-labell ">رابط المدينة</label>
                <input type="text" name="href" value="{{$cities->href}}" class="form-controll">
            </div>
            {{-- <div class="col-md-12 mb-3">
                <label><strong> المحتوى النصي(وصف):</strong></label>
                <textarea class="ckeditor form-controll" name="description">{!! $cities->description !!}</textarea>
            </div> --}}
