...
@include('dash-board.layout.sidebar')
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg overflow-x-hidden">
        ...
        <div class="row backgroundW p-4 m-3">
            ...
            <form action="{{ route('fixedsitesmain.update' , $fixedsitestmain->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                
                <div class="form-group">
                    <label for="exampleFormControlInput1">اسم الموقع</label>
                    <input type="text" class="form-controll" name="name" value="{{$fixedsitestmain->name}}">
                </div>
                
                <div class="form-group">
                    <label for="exampleFormControlInput1">رايط الموقع</label>
                    <input type="text" class="form-controll" name="href"  value="{{$fixedsitestmain->link}}">
                </div>

                <div class="form-group">
                    <label for="exampleFormControlInput1">صورة</label>
                    <input type="file" name="photo" class="form-controll">
                    <img src="{{url('../public/uploading/' . $fixedsitestmain->img)}}" alt="{{$fixedsitestmain->link}}">
